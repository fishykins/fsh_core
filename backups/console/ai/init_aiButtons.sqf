...
FUNC(sliderColChanged) = {
    ...
    private _object = GVAR(unit);
    if (GVAR(unit) isEqualto objNull) then {
        _object = GVAR(group);
    };

    private _colour = _object call FUNC(getObjectColour);
    //private _trackingLevel = _object getVariable [QGVARMAIN(trackingLevel), 0];
    _colour set [_index, _amount/10];

    [_object,_colour] call FUNC(setColours);
};

...
//Get all connected players yet to be loaded
{
    if (!(_x getVariable [QGVAR(loaded), false])) then {
        private _data = [_x, _world, _session] call fsh_fnc_dbLoadPlayer;
...
FUNC(pauseMenuOpened) = {
    [player] call fsh_fnc_dbSavePlayer;
};

[_loopTime] execFSM QPATHTOF(save_players.fsm);
